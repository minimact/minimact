...
// CSS Properties type (compatible with React.CSSProperties but framework-agnostic)
...
export interface LifecycleStateConfig<TState extends string = string> {
  ...
  // Templates per state (any renderable content)
  templates?: Partial<Record<TState, any>>;

  // Transition rules (state machine graph)
  // If not provided, all transitions are allowed
  ...
  // Timing configuration (auto-transition after duration in ms)
  durations?: Partial<Record<TState, number>>;

  // Animation easings per state
...
export interface TransitionHistoryEntry<TState extends string = string> {
  ...
  to: TState;
  timestamp: number;
  durationInState: number; // How long we stayed in 'from' state
}

/**
 * LifecycleStateTracker - The core state machine implementation
 */
export class LifecycleStateTracker<TState extends string = string> {
  private currentState: TState;
  private previousState?: TState;
  private config: LifecycleStateConfig<TState>;

  // Timing
  private stateStartTime: number;
  private autoTransitionTimer?: number;

  // History
  private transitionHistory: TransitionHistoryEntry<TState>[] = [];
  ...
  transitionTo(newState: TState): boolean {
    ...
    if (!this.canTransitionTo(newState)) {
      console.warn(
        `[LifecycleStateTracker] Invalid transition: ${this.currentState} → ${newState}`
      );
      return false;
    }
   ...
   */
  predictNextState(): { state: TState; confidence: number } | null {
    ...
    const currentStateTransitions = this.transitionHistory.filter(
      entry => entry.from === this.currentState
    );
    ...
    const transitionCounts = new Map<TState, number>();
    for (const nextState of nextStates) {
      const count = currentStateTransitions.filter(
        entry => entry.to === nextState
      ).length;
  ...
  private scheduleAutoTransition(duration: number): void {
    this.autoTransitionTimer = window.setTimeout(() => {
      ...
      if (nextStates.length === 1) {
        ...
        console.warn(
          `[LifecycleStateTracker] Auto-transition from '${this.currentState}' has ${nextStates.length} options. ` +
  ...
  private validateConfig(): void {
    ...
    if (!this.config.states.includes(this.config.defaultState)) {
      throw new Error(
        `[LifecycleStateTracker] defaultState "${this.config.defaultState}" not in states array`
      );
    }
    ...
    if (this.config.states.length === 0) {
      throw new Error(
        `[LifecycleStateTracker] states array cannot be empty`
      );
    }
    ...
    if (stateSet.size !== this.config.states.length) {
      console.warn(
        `[LifecycleStateTracker] Duplicate states detected in states array`
      );
    }
    ...
    if (this.config.transitions) {
      for (const [from, toStates] of Object.entries(this.config.transitions)) {
        if (!this.config.states.includes(from as TState)) {
          console.warn(
            `[LifecycleStateTracker] Transition rule for unknown state: ${from}`
          );
        }

        for (const to of toStates as TState[]) {
          if (!this.config.states.includes(to)) {
            console.warn(
              `[LifecycleStateTracker] Transition ${from} → ${to} references unknown state: ${to}`
            );
          }
        }
      }
      ...
      const unreachable = this.config.states.filter(
        state => !reachable.has(state)
      );

      if (unreachable.length > 0) {
        console.warn(
          `[LifecycleStateTracker] Unreachable states detected: ${unreachable.join(', ')}`
        );
      }
   ...
   */
  private findReachableStates(): Set<TState> {
    if (!this.config.transitions) {
      return new Set(this.config.states);
    }

    const reachable = new Set<TState>();
    const queue: TState[] = [this.config.defaultState];
  ...
  hasTransitionLoops(): boolean {
    if (!this.config.transitions) return false;

    const visited = new Set<TState>();
    const recursionStack = new Set<TState>();

    const dfs = (state: TState): boolean => {
      visited.add(state);
