 ...
 */
function useMvcState(propertyName, options) {
    // Get current component context (set by Minimact runtime)
    const ctx = window.__MINIMACT_CONTEXT__ || currentContext;
    if (!ctx) {
    ...
    if (!wrapper) {
        console.warn(`[MVC Bridge] No ViewModel found. ` +
    ...
    const setState = (newValue) => {
        ...
        if (syncStrategy === 'immediate') {
            ...
            debouncedSyncToServer(context, stateKey, actualNewValue, propertyName, options?.syncDelay || 300);
        }
 ...
 */
function isMvcPropertyMutable(propertyName) {
    const wrapper = window.__MINIMACT_VIEWMODEL__;
 ...
 */
function syncToServer(context, stateKey, value, propertyName) {
    if (!context.signalR) {
    ...
    context.signalR.updateComponentState(context.componentId, stateKey, value)
        .catch((err) => {
        console.error(`[MVC Bridge] Failed to sync MVC state '${propertyName}' to server:`, err);
        // Check if error is due to immutability violation
        if (err.message && err.message.includes('immutable')) {
            console.error(`[MVC Bridge] 🚨 SECURITY: Attempted to modify immutable state '${propertyName}'. ` +
...
function debouncedSyncToServer(context, stateKey, value, propertyName, delay) {
 ...
 * @internal
 */
function setMvcContext(context) {
...
exports.clearMvcContext = clearMvcContext;
exports.flushMvcState = flushMvcState;
exports.getMvcStateMetadata = getMvcStateMetadata;
exports.isMvcPropertyMutable = isMvcPropertyMutable;
exports.resetMvcStateIndex = resetMvcStateIndex;
exports.setMvcContext = setMvcContext;
exports.useMvcState = useMvcState;
exports.useMvcViewModel = useMvcViewModel;
