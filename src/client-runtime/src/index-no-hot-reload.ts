...
// REMOVED: import { HotReloadManager } from './hot-reload';
...
/**
 * Main Minimact client runtime (WITHOUT hot-reload for size testing)
 * Orchestrates SignalM (lightweight WebSocket), DOM patching, state management, and hydration
 *
 * Bundle size: Testing without hot-reload to measure savings
 */
export class Minimact {
  ...
  // REMOVED: private hotReload: HotReloadManager | null = null;
  ...
  constructor(rootElement: HTMLElement | string = document.body, options: MinimactOptions = {}) {
    ...
    this.options = {
      ...
      enableHotReload: false, // DISABLED for size test
    ...
    this.log('Minimact initialized (NO HOT-RELOAD)', { rootElement: this.rootElement, options: this.options });
  }
...
// REMOVED: export { HotReloadManager } from './hot-reload';
