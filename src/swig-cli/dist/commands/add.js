 ...
 */
async function addCommand(pageName, routePath, options = {}) {
    const projectPath = options.project || process.cwd();
    // Interactive mode if pageName or routePath not provided
    if (!pageName || !routePath) {
        console.log(chalk_1.default.bold.cyan('✨ Add a new page to your Minimact project\n'));
        const answers = await (0, prompts_1.default)([
            ...
            },
            {
                ...
                validate: (value) => value.length > 0 ? true : 'Route path is required'
            },
            {
                type: 'confirm',
                name: 'useMvc',
    ...
    if (/[^a-zA-Z0-9_]/.test(pageName)) {
        console.error(chalk_1.default.red('❌ Invalid page name!'));
        console.error(chalk_1.default.yellow(`   Page name "${pageName}" contains invalid characters.`));
        console.error(chalk_1.default.gray('   Use PascalCase or underscores (e.g., ProductDetails, Product_Details)'));
        console.error(chalk_1.default.gray('   Avoid dashes, spaces, or special characters'));
        process.exit(1);
    }
    if (/^\d/.test(pageName)) {
        console.error(chalk_1.default.red('❌ Invalid page name!'));
        console.error(chalk_1.default.yellow(`   Page name "${pageName}" cannot start with a number.`));
        process.exit(1);
    }
    console.log(chalk_1.default.bold(`📄 Adding new page: ${chalk_1.default.cyan(pageName)}`));
    console.log(`   Route: ${chalk_1.default.green(`/${routePath}`)}`);
    console.log(`   Mode: ${options.mvc ? chalk_1.default.magenta('MVC Bridge') : chalk_1.default.blue('Standard')}`);
    console.log('');
    try {
        ...
        const controllerContent = generateController(projectName, controllerName, pageClassName, routePath, viewModelName);
        await fs.writeFile(controllerPath, controllerContent, 'utf-8');
        console.log(chalk_1.default.green(`   ✅ Created controller: ${path.relative(projectPath, controllerPath)}`));
        ...
        if (options.mvc && viewModelName) {
            ...
            console.log(chalk_1.default.green(`   ✅ Created ViewModel: ${path.relative(projectPath, viewModelPath)}`));
        }
        // Generate Page (TSX)
        ...
        console.log(chalk_1.default.green(`   ✅ Created page: ${path.relative(projectPath, pagePath)}`));
        // Transpile the page
        console.log(chalk_1.default.gray('   🔄 Transpiling page to C#...'));
        const transpiler = new swig_shared_1.TranspilerService();
        const result = await transpiler.transpileFile(pagePath);
        if (result.success) {
            console.log(chalk_1.default.green(`   ✅ Transpiled to C# in ${result.duration}ms`));
        }
        else {
            console.warn(chalk_1.default.yellow(`   ⚠️  Transpilation failed: ${result.error}`));
        }
        console.log(chalk_1.default.green.bold('\n✅ Page added successfully!'));
        console.log('');
        console.log(chalk_1.default.bold('Next steps:'));
        console.log(`   ${chalk_1.default.gray('•')} Edit ${chalk_1.default.cyan(path.relative(projectPath, pagePath))}`);
        if (options.mvc && viewModelName) {
            console.log(`   ${chalk_1.default.gray('•')} Update ${chalk_1.default.cyan(path.relative(projectPath, path.join(viewModelsDir, `${viewModelName}.cs`)))}`);
        }
        console.log(`   ${chalk_1.default.gray('•')} Run ${chalk_1.default.cyan('swig watch')} to auto-transpile changes`);
        console.log(`   ${chalk_1.default.gray('•')} Navigate to ${chalk_1.default.cyan(`http://localhost:5000/${routePath}`)}`);
    }
    catch (error) {
        console.error(chalk_1.default.red('❌ Failed to add page:'), error);
        throw error;
    }
}
/**
 ...
 */
function generateController(projectName, controllerName, pageClassName, routePath, viewModelName) {
 ...
 */
function generateViewModel(projectName, viewModelName) {
 ...
 */
function generatePage(pageClassName, useMvc, viewModelName) {
