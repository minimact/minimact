...
function useDynamicState(initialState) {
    ...
    const orderBindingsRef = useRef(new Map());
    const updaterRef = useRef(new ValueUpdater());
     ...
     */
    const registerBinding = useCallback((selector, fn) => {
     ...
     */
    const registerOrderBinding = useCallback((containerSelector, fn) => {
        // Track dependencies
     ...
     * Update state
     */
    const updateState = useCallback((updaterOrPartial) => {
...
export { PHILOSOPHY, VERSION, ValueUpdater, hasPathChanged, shallowEqual, trackDependencies, useDynamicState };
//# sourceMappingURL=minimact-dynamic.esm.js.map
