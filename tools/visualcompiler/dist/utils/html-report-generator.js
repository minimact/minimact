...
export class HTMLReportGenerator {
    ...
    buildHTMLContent(report, options) {
...
<body>
    ...
    buildHeader(report, projectName, timestamp) {
    ...
    buildScreenshotSection(options) {
    ...
    buildSeverityIssuesList(issues, severity) {
        if (issues.length === 0) {
    ...
    buildAlternativeFixes(alternatives) {
        return `
    ...
    buildDetailedBreakdown(report) {
        return `
    <section class="detailed-section">
        <h2>📊 Detailed Analysis by Resolution</h2>
        ${report.reports.map((resReport) => this.buildResolutionReport(resReport)).join('')}
    </section>`;
    }
    buildResolutionReport(resReport) {
