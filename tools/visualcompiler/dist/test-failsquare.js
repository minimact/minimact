...
class FailSquareAnalyzer {
    ...
    async analyzeAllComponents() {
        ...
        for (const component of components) {
            ...
            try {
                const report = await this.compiler.analyzeContent(component.htmlContent, `failsquare-${component.name.toLowerCase()}.html`);
                // Log summary
                ...
                for (const resReport of report.reports) {
                    const criticalIssues = resReport.issues.filter(issue => issue.type === 'error' && (issue.code.includes('E101') || issue.code.includes('E301')));
     ...
     */
    async createFailSquareComponentTests() {
        return [
            {
                name: 'FailSquareCard',
