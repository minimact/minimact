...
class VisualCompilerCLI {
  ...
  static async watchCommand(args: string[]) {
  ...
  static async analyzeCommand(args: string[]) {
    if (args.length === 0) {
  ...
  static async analyzePageCommand(args: string[]) {
    if (args.length === 0) {
    ...
    if (generateHTMLReport) console.log('🌐 HTML report will be generated');
    if (captureScreenshots) console.log('📷 Screenshots will be captured');
    ...
    try {
      ...
      const report = await compiler.analyzeContent(
        htmlContent,
        `page-${path.basename(pagePath, '.tsx').toLowerCase()}.html`,
        {
          generateHTMLReport,
          captureScreenshots,
