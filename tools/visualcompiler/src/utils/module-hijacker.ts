...
const ModuleType = Module as any;

export class ModuleHijacker {
  private originalLoad: any;
  private isActive = false;
  private config: HijackConfig;

  constructor(config: Partial<HijackConfig> = {}) {
  ...
  activate(): void {
    ...
    ModuleType._load = function(request: string, parent: NodeModule, isMain?: boolean) {
   ...
   */
  setupMockEnvironment(scenario: 'dashboard' | 'profile' | 'submit' | 'custom', customData?: any): void {
    resetMockState();
    ...
    injectContext('TabNavigationContext', {
      ...
      navigateToTab: (path: string, title: string) => {
   ...
   */
  injectTestState(key: string, value: any): void {
    injectState(key, value);
  }
   ...
   */
  injectTestContext(key: string, value: any): void {
 ...
 */
export async function withHijackedModules<T>(
  scenario: 'dashboard' | 'profile' | 'submit' | 'custom',
  fn: () => Promise<T> | T,
  customData?: any
): Promise<T> {
