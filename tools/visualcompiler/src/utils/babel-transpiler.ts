...
export class BabelTranspiler {
   ...
   */
  async transpileFile(options: TranspileOptions): Promise<string> {
    ...
    if (outputDir) {
      const outputFile = path.join(outputDir,
        path.basename(sourceFile).replace(/\.tsx?$/, '.js')
      );
   ...
   */
  private replaceReactImports(sourceCode: string): string {
    // Replace React imports with our shim
    ...
    shimmed = shimmed.replace(
      ...
      `import React from '${this.getShimPath('react-shim.js')}';`
    );
    ...
    shimmed = shimmed.replace(
      ...
      `import { useAuth } from '${this.getShimPath('react-shim.js')}';`
    );

    // Replace useTabNavigation hooks
    shimmed = shimmed.replace(
      /import\s+\{[^}]*useTabNavigation[^}]*\}\s+from\s+['"][^'"]*['"];?/g,
      `import { useTabNavigation } from '${this.getShimPath('react-shim.js')}';`
    );
   ...
   */
  private getShimPath(shimFile: string): string {
   ...
   */
  async createTemporaryTranspiledFile(sourceFile: string): Promise<string> {
   ...
   */
  async transpileEntireCodebase(): Promise<string> {
   ...
   */
  private replaceImportsForCodebase(sourceCode: string, currentFile: string, baseDir: string): string {
    let shimmed = sourceCode;
