...
export class ReactRenderer {
  ...
  constructor(tempDir: string = './temp-rendered') {
   ...
   */
  async renderComponentToHTML(componentInfo: ReactComponentInfo): Promise<string> {
   ...
   */
  private generateNextJSPage(componentInfo: ReactComponentInfo): string {
...
`;
   ...
   */
  private async createNextJSConfig(): Promise<void> {
    const nextConfig = `
...
`;
   ...
   */
  private async buildAndExportHTML(componentName: string): Promise<string> {
...
<body>
   ...
   */
  async renderStoryToHTML(storyPath: string, storyName: string): Promise<string> {
    // Read the story file
...
<body class="bg-gray-50 p-4">
  ...
  private extractComponentNameFromStory(content: string): string {
    const match = content.match(/export\s+default\s+{[^}]*component:\s*(\w+)/);
    return match ? match[1] : 'UnknownComponent';
  }

  private extractPropsFromStory(content: string, storyName: string): Record<string, any> {
  ...
  private generateHTMLFromStory(componentName: string, storyName: string, props: Record<string, any>): string {
