...
### 🌳 Lifted State Components - Revolutionary State Architecture

**ALL child state automatically lives in the parent. Zero prop drilling. Perfect prediction.**

```typescript
// Parent automatically owns ALL child state
function Dashboard() {
  const userEditing = state["UserProfile.isEditing"];  // Read any child state!
  const cartCount = state["ShoppingCart.items"].length;  // Coordinate freely!
...
// Child accesses without knowing about namespace
function UserProfile() {
  const isEditing = state.isEditing;  // Auto-prefixed: "UserProfile.isEditing"
  setState('isEditing', true);         // Updates parent state!
}
```
...
- ✅ Parent observes ANY child state
- ✅ Perfect prediction (full state tree visible)
- ✅ Trivial debugging (flat state structure)
- ✅ Hot reload preserves state (it's in parent!)
- ✅ Cross-component coordination built-in

### 🔒 useProtectedState - Encapsulation for Lifted State

**Lifted state with access control - state that parents can't touch:**
...
**Key Innovation**: State is still lifted (visible to DevTools, prediction, time travel), but parent **cannot** access it at runtime.

**Use for**: Internal caches, animation state, debounce timers, validation internals.
...
### 🎨 Minimact Swig - Desktop IDE for Minimact

**Complete Electron-based development environment with real-time component inspection.**

<p align="center">
  <img src="./docs/assets/swig-screenshot.png" alt="Minimact Swig IDE" width="800">
</p>

Minimact Swig provides a complete desktop IDE experience for Minimact development:

**Features:**
- ✅ **Monaco Editor** - Full TypeScript/TSX support with IntelliSense
- ✅ **Auto-transpilation** - Watch mode with instant TSX → C# compilation
- ✅ **Real-time Component Inspector** - SignalR connection for live state inspection
- ✅ **Integrated Terminal** - Run builds and commands without switching apps
- ✅ **File Tree** - Navigate your project structure
- ✅ **One-click Operations** - Create project, build, run, view in browser
- ✅ **Hot Reload Integration** - See changes instantly
- ✅ **Prediction Analytics** - Visual cache hit indicators and metrics
...
**Benefits:**
- Complete IDE experience without VS Code
- Real-time visibility into component state and predictions
- Visual debugging with cache hit/miss overlay
- Performance metrics dashboard
- Seamless workflow from code to running app

**Documentation**: See [MINIMACT_SWIG_ELECTRON_PLAN.md](./docs/MINIMACT_SWIG_ELECTRON_PLAN.md)
...
Minimact consists of seven main components working in harmony:
...
- **Handles `<Component>` wrapper** for lifted state
...
- **VComponentWrapper** for lifted state pattern
- **ComponentTypeRegistry** for dynamic instantiation
- **StateManager** with namespace prefixing
- **Protected state enforcement** via runtime validation
...
- **State synchronization** to server (useState, useDomElementState)
- Fallback for no-JS scenarios

### 5. **Lifted State System**
- `<Component>` wrapper for automatic state lifting
- All child state lives in parent with namespace prefixes
- Zero prop drilling, no callback boilerplate
- Parent can read/write any child state
- **useProtectedState** for access-controlled state
- Perfect prediction with full state tree visibility
- Hot reload preserves state (it's in parent!)

### 6. **Minimact Punch** (Extensions)
- **useDomElementState** hook - DOM as reactive data source
- 80+ DOM properties as reactive state
- IntersectionObserver, MutationObserver, ResizeObserver integration
- Collection queries and statistical aggregates
- Automatic synchronization to server
- MES (Minimact Extension Standards) certified

### 7. **Minimact Swig** (Developer Tools)
- Electron-based desktop IDE
- Monaco editor with full TSX support
- Real-time component inspector via SignalR
- Auto-transpilation watch mode
- Integrated terminal and file tree
- Visual prediction analytics dashboard
...
- [x] **Template prediction system (Phases 1-9)** - 95-98% coverage, 98% memory reduction
...
- [x] **C# runtime and component base classes**
  ...
  - [x] VComponentWrapper for lifted state
  - [x] ComponentTypeRegistry for dynamic instantiation
...
- [x] **Client library** (JavaScript runtime)
  ...
  - [x] Template renderer
  - [x] State synchronization (client changes sync to server)

### 🎉 Recently Completed Advanced Features

- [x] **🌳 Lifted State Component System** - Revolutionary state architecture
  - [x] `<Component>` wrapper element in TSX
  - [x] Automatic state namespacing (e.g., "UserProfile.isEditing")
  - [x] VComponentWrapper VNode type
  - [x] Parent can read/write all child state
  - [x] Zero prop drilling, no callback boilerplate
  - [x] Perfect prediction (full state tree visibility)
  - [x] Hot reload preserves state (it's in parent!)
  - **Documentation**: [LIFTED_STATE_COMPONENT_SYSTEM.md](./docs/LIFTED_STATE_COMPONENT_SYSTEM.md)

- [x] **🔒 useProtectedState Hook** - Encapsulation for lifted state
  - [x] State lifted to parent but access-controlled
  - [x] Runtime validation with [Mutable] attribute
  - [x] Visible to DevTools, prediction, time travel
  - [x] Parent cannot read or write protected state
  - **Documentation**: [USE_PROTECTED_STATE.md](./docs/USE_PROTECTED_STATE.md)

- [x] **🎨 Minimact Swig** - Desktop IDE for Minimact development
  - [x] Electron app with Monaco editor
  - [x] Real-time component inspector via SignalR
  - [x] Auto-transpilation watch mode
  - [x] Integrated terminal and file tree
  - [x] One-click project creation, build, run
  - [x] Visual prediction analytics dashboard
  - [x] Performance metrics and cache hit overlay
  - **Documentation**: [MINIMACT_SWIG_ELECTRON_PLAN.md](./docs/MINIMACT_SWIG_ELECTRON_PLAN.md)

- [x] **📐 Template Patch System** - Build-time template extraction
  - [x] Babel extracts templates from JSX patterns
  - [x] Loop templates for `.map()` with full structure
  - [x] Expression templates for computed values
  - [x] Structural templates for conditional rendering
  - [x] 100% coverage from first render (zero cold start)
  - [x] ~2KB per component vs ~100KB for cached patches
  - **Documentation**: [TEMPLATE_PATCH_SYSTEM.md](./docs/TEMPLATE_PATCH_SYSTEM.md)

- [x] **🍹 Minimact Punch (Base Features)** - DOM as reactive data source
  - [x] useDomElementState hook with 80+ DOM properties
  - [x] IntersectionObserver, MutationObserver, ResizeObserver integration
  - [x] Collection queries and statistical aggregates
  - [x] Automatic state synchronization to server
  - [x] MES (Minimact Extension Standards) Silver certification
  - **Documentation**: [USEDOMELEMENTSTATE_IMPLEMENTATION_PLAN.md](./docs/USEDOMELEMENTSTATE_IMPLEMENTATION_PLAN.md)
...
- [x] **Template library**
  - DefaultLayout, SidebarLayout, AuthLayout, AdminLayout
...
- [ ] **Minimact Punch Advanced Features** (Parts 2-5)
  - [ ] Pseudo-state reactivity (`:hover`, `:focus`, etc.)
  - [ ] Theme and breakpoint awareness
  - [ ] Temporal features (state history, trends)
  - [ ] Lifecycle state machines

See [VISION.md](./src/VISION.md), [PHASES_1_TO_9_COMPLETION_SUMMARY.md](./docs/PHASES_1_TO_9_COMPLETION_SUMMARY.md), and [MINIMACT_COMPLETE_ARCHITECTURE.md](./docs/MINIMACT_COMPLETE_ARCHITECTURE.md) for details.
