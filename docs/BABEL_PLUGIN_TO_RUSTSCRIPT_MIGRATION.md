# Babel Plugin Minimact → RustScript Migration Plan

**Status**: Planning
**Target**: Migrate babel-plugin-minimact from JavaScript to RustScript
...
### Target State
A RustScript `writer` plugin that:
...
This maps to RustScript's `writer` API (Section 16 of spec):
...
| Current JS Module | RustScript Equivalent |
|-------------------|----------------------|
...
## 4. RustScript Gaps & Enhancements Needed

### 4.1 Critical Gaps

#### A. Multiple Output Files
RustScript `writer` returns a single `Str`. babel-plugin-minimact generates 5+ files.

**Solution**: Extend RustScript with multi-file output:
...
Current plugin reads/writes files directly. RustScript has `use fs;` but:
...
```rustscript
// RustScript plugin returns structured output
...
Heavy use of JSON for output. RustScript has `use json;` but needs:
...
**Solution**: Not supported in RustScript (Section 18.1). Must use string methods:
...
**Current**: RustScript has basic string methods. May need extensions:
- `s.split(",")` → `Vec<Str>`
...
**Solution**: Already handled by RustScript's U-AST mapping (Section 7).
...
**Solution**: RustScript doesn't support async, but can detect `async` flag:
...
1. **Set up RustScript project structure**
   ```
   minimact-transpiler/
   ├── src/
...
## 7. Required RustScript Enhancements
...
| Missing AST nodes | High | Extend RustScript mappings |
...
2. **Prioritize RustScript enhancements** needed
...
*Generated by Claude Code - Migration planning document for babel-plugin-minimact to RustScript*
